            ...
            <TabsContent value="exercises" className="space-y-4">
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>Follow along with the exercises in this workout</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="w-full bg-muted rounded-full h-2 relative">
                    ...
                    {video.exercises.map((exercise) => {
                      ...
                      const widthPercent =
                        ((exercise.endTime - exercise.startTime) / video.duration) * 100;
                      const isActive =
                  ...
                  {getCurrentExercise() && (
                    <div className="p-4 rounded-lg bg-primary/10 border border-primary/20">
                      <div className="flex justify-between items-center mb-2">
                        <h3 className="font-semibold">Current: {getCurrentExercise()?.name}</h3>
                        <Badge variant="outline">{getCurrentExercise()?.muscleGroup}</Badge>
                      </div>
                      <div className="flex items-center justify-between text-sm text-muted-foreground">
                        <span>
                          {formatTime(getCurrentExercise()?.startTime || 0)} -{" "}
                          {formatTime(getCurrentExercise()?.endTime || 0)}
                        </span>
                  ...
                  <div className="space-y-2">
                    {video.exercises.map((exercise) => (
                      <div
                        key={`${exercise.name}-${exercise.startTime}-${exercise.endTime}`}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h4 className="text-sm font-medium line-clamp-2">
                      15 Min Abs Workout - No Equipment
                    </h4>
